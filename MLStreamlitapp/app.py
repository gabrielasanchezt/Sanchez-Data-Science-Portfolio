...
if data is not None:
    ...
    if data.shape[1] < 2:
        ...
        # Drop non-numeric columns (e.g., SMILES strings, names)
        X = X.select_dtypes(include=["number"])

        if X.shape[1] < data.shape[1] - 1:
            st.warning("Some non-numeric features were dropped automatically. Consider encoding them if needed.")
